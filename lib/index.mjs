...
export class SaniLine {
  ...
  sanitizeLine(line, lineNumber = 1, language = null) {
    ...
    // 1. Trojan Source (CVE-2021-42574) & Invisible Unicode Shield
    ...
    // 2. Hardcoded Secrets & High-Entropy Token Redactor (CWE-798)
    ...
    // 3. RCE & Command Injection (CWE-78, CWE-95)
    ...
    if (/\bcreateHash\s*\(\s*['"](md5|sha1)['"]\s*\)/i.test(currentLine)) {
      violations.push({
        rule_id: "SL-CRY-003",
      ...
      if (this.action === SanitizeAction.AUTOPATCH) {
        currentLine = currentLine.replace(/\bcreateHash\s*\(\s*['"](md5|sha1)['"]\s*\)/gi, "createHash('sha256')");
        modified = true;
      }
    }
