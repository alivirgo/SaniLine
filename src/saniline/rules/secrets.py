...
class HardcodedSecretRule(BaseRule):
    ...
    # Explicit high-confidence signatures with bounded patterns
    SIGNATURES = [
        ...
        ("OpenAI Key", re.compile(r"\b(sk-[a-zA-Z0-9]{20,128}|sk-proj-[a-zA-Z0-9_-]{20,128})\b"), "[REDACTED_SECRET:OPENAI_API_KEY]"),
        ("Anthropic Key", re.compile(r"\b(sk-ant-[a-zA-Z0-9_-]{20,128})\b"), "[REDACTED_SECRET:ANTHROPIC_API_KEY]"),
        ("Google Cloud Key", re.compile(r"\b(AIza[0-9A-Za-z_-]{35})\b"), "[REDACTED_SECRET:GOOGLE_API_KEY]"),
        ("Stripe Secret Key", re.compile(r"\b(sk_live_[0-9a-zA-Z]{24,128})\b"), "[REDACTED_SECRET:STRIPE_SECRET_KEY]"),
        ("Slack Token", re.compile(r"\b(xox[baprs]-[0-9a-zA-Z]{10,48})\b"), "[REDACTED_SECRET:SLACK_TOKEN]"),
        ("Supabase Token", re.compile(r"\b(sbp_[a-zA-Z0-9]{40,128})\b"), "[REDACTED_SECRET:SUPABASE_TOKEN]"),
    ...
    # Assignment patterns indicative of sensitive credentials (bounded quantifier prevents ReDoS)
    ASSIGNMENT_PATTERN = re.compile(
        r"""(?i)\b(password|passwd|secret|api_?key|access_?token|auth_?token|client_?secret|bearer)\s*[:=]\s*(['"])([^'"]{8,256})\2"""
    )
    ...
    def _generate_patch(self, line: str, match: re.Match[str], snippet: str) -> str:
